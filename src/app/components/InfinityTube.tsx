...
function FlowingInfinityTube() {
  ...
  const particlesGeo = useMemo(() => {
    ...
    const c1 = new THREE.Color("#2dd4ff"); // cyan-blue
    const c2 = new THREE.Color("#3b82f6"); // deep blue
    const c3 = new THREE.Color("#60a5fa"); // soft light blue
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const t = u0[i];
      const mixed = c1.clone().lerp(c2, (Math.sin(t * Math.PI * 2) + 1) / 2).lerp(c3, t * 0.5);
  ...
  return (
    <group rotation={[0.4, 0.4, 0.4]} position={[0, 0, 0]} scale={1.4}>
      {/* Flowing particles */}
